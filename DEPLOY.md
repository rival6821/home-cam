...
**대부분의 경우 컨테이너 재시작은 필요 없다** — 정적 파일이라 Caddy가 요청마다
디스크에서 새로 읽는다. 반영됐는지는 브라우저에서 강력 새로고침(iOS Safari는
캐시를 길게 들고 있을 수 있으니 설정 화면을 리셋하거나 캐시 무효화
쿼리스트링으로 확인) 후 접속 화면(§4)으로 확인한다.

**단, `git pull` 후에도 이전 내용이 그대로 나오면 컨테이너를 재시작한다**:
...
원인: Docker의 "파일 단위 바인드 마운트"는 경로가 아니라 그 파일의 **inode**를
컨테이너에 연결한다. 그런데 `git`이 파일을 갱신할 때는 대부분 기존 파일을
그 자리에서 덮어쓰지 않고 지운 뒤 새로 만들기 때문에, 호스트 경로는 같아도
inode가 바뀐다. 이 경우 컨테이너는 옛 inode를 계속 붙들고 있어 `git pull`이
전혀 반영되지 않는데, 위의 "요청마다 디스크에서 새로 읽는다"는 설명은 파일이
같은 inode를 유지한 채 제자리에서 수정될 때만 성립한다.

의심되는 경우 호스트와 컨테이너 안 inode를 비교해 확인할 수 있다:

```bash
ls -i ~/home-cam/camera.html
docker exec homecam-caddy ls -i /var/www/homecam/camera.html
# 두 inode 번호가 다르면 이 문제다 → 위 재시작으로 해결
```
